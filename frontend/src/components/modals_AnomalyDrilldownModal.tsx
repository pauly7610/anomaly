...
export default function AnomalyDrilldownModal({ open, onClose, anomaly, loading, error }: { open: boolean; onClose: () => void; anomaly: any, loading?: boolean, error?: string | null }) {
  if (!open) return null;
  return (
      ...
      <div className="bg-white rounded shadow-lg p-6 max-w-lg w-full relative">
        ...
        {loading ? (
          ...
          <div className="text-sm space-y-1">
            {Object.entries(anomaly).map(([k, v]) => (
              <div key={k}><b>{k}:</b> {String(v)}</div>
            ))}
          </div>
        ) : (
          <div className="text-gray-500 text-sm py-4">No anomaly data found.</div>
        )}
      </div>
    </div>
  );
